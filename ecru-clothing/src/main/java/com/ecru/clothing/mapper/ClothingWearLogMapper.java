...
public interface ClothingWearLogMapper extends BaseMapper<ClothingWearLog> {
    ...
    @Select("SELECT * FROM clothing_wear_log WHERE clothing_id = #{clothingId} ORDER BY worn_at DESC LIMIT #{limit}")
    List<ClothingWearLog> selectRecentWearLogs(
            @Param("clothingId") Long clothingId,
            @Param("limit") Integer limit
    );

    @Select("SELECT COUNT(*) FROM clothing_wear_log WHERE clothing_id = #{clothingId} AND worn_at BETWEEN #{startDate} AND #{endDate}")
